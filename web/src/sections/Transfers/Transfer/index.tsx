...
export const TransferInfo: React.FC<TransferProps> = observer(({ transfer }) => {
      ...
      <div className={styles.state}>
        <div className={styles.filename}>
          ...
          <Tooltip content={transfer.fileName}>
            <span className="mobileHidden">{formatFileName(transfer.fileName)}</span>
            <span className="desktopHidden">{formatFileName(transfer.fileName, 10)}</span>
          </Tooltip>
        </div>
        {offset ? <progress value={offset / transfer.fileSize} max={1} /> : null}
